...
public class Day8 {
    public static void main(String... args) {
        ...
        try (Stream<String> stream = Files.lines(Paths.get("C:\\Users\\Skazzi\\IdeaProjects\\Santa\\src\\in8.txt"))) {
            list = stream
        ...
        int memory1 = list
                ...
                .mapToInt(x -> x.length())
                .sum();
        System.out.println("M: " + memory1);

        // encoded
        int encode = list.stream()
                .map(x -> x.replace("\\", "\\\\"))
                .map(x -> x.replace("\"", "\\\""))
                .map(x -> "\"" + x + "\"")
                .mapToInt(x -> x.length())
        ...
        // part 1:
        System.out.println("Answer1: " + (sum - memory1));
        // part 2:
        System.out.println("Answer2: " + (encode - sum));
    }
}
